...
```
Exercise 1__: How many cells & genes have been removed by this filtering? Do you agree with the 2000 detected genes threshold?
...
the same technical noise as endogenous transcripts (Risso & teichman REFERENCE).
...
from the failure of mRNAs failing to be reversed transcribed (REF-me). Reverse transcription
...
DANB_fit <- NBumiFitModel(usoskin1) # DANB is fit to the raw count matrix
# Check the fit of the model
par(mfrow=c(1,2))
stats <- NBumiCheckFitFS(count_mat, DANB_fit)
c(stats$gene_error,stats$cell_error)
...
score <- abs(pca$x[,c(1,2)]) # calculate loadings for components 1 and 2
names(score) <- rownames(expr_matrix)
score <- score[order(-score)]
