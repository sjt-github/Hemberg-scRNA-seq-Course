...
```{r synthetic-roc-curve-ks, fig.cap = "Different distributions of read counts for a single genes across 100 cells based on the Poisson-Beta model corresponding to different paramete sets"}
...
```{r synthetic-roc-curve-deseq, fig.cap = "Different distributions of read counts for a single genes across 100 cells based on the Poisson-Beta model corresponding to different paramete sets"}
...
nCells <- 100
mult <- 2^(rnorm(nGenes, 0, 2))
synData <- scRNA.seq.course::GeneratePoiBetaSamples(ks = 10^(rnorm(nGenes, 3, .5)),
                                  as = 10^(rnorm(nGenes, -1, .5)),
                                  bs = 10^(rnorm(nGenes, 0, .5)),
                                  mult,
                                  nGenes,
...
__Exercise:__ Run the KS, DESeq2 and SCDE methods on the synthetic
data that was just generated and compare the AUC values. Which method
...
# do NOT delete this
# Comparison of KS-test and DESeq2
nGenes <- 1e2
nCells <- 1e2
mult <- 2^(rnorm(nGenes, 0, 2))
synData <- scRNA.seq.course::GeneratePoiBetaSamples(ks = 10^(rnorm(nGenes, 3, .5)),
...
g2 <- synData$sample2
pVals <- rep(1, nGenes)
for (i in 1:nGenes) { pVals[i] <- ks.test(g[i,], g2[i,])$p.value*nGenes }
aucKSFixed <- ROCR::performance(ROCR::prediction(pVals, as.numeric(abs(log2(mult))<=2)),"auc")@y.values[[1]]
cnts <- cbind(g, g2)
cond <- factor(c(rep("A", ncol(g)),
                 rep("B", ncol(g2))))
dds <- DESeq2::DESeqDataSetFromMatrix(cnts + 1, DataFrame(cond), ~ cond)
dds <- DESeq2::DESeq(dds)
resDESeq <- results(dds)
aucDESeq2Fixed <- ROCR::performance(prediction(resDESeq$padj,
                        as.numeric(abs(log2(mult))<=2)),"auc")@y.values[[1]]
```
...
__Exercise__: Generate synthetic data 100 times and determine the
stability and consistency of the different methods. Note that it will
take a considerable amount of time to run DESe2 and SCDE.
