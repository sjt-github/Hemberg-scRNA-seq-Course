...
By default scater will use 5-times the median absolute difference from the median value as a threshold to filter cells according to the number of detected genes, total counts. In addition to filtering out cells with >80% of counts assigned to control genes and any cells that have been marked as controls.
...
The distribution relatively flat indicating good coverage of the full transcriptome of these cells. However, there are several spike-ins in the top 15 genes which suggests a greater dilution of the spike-ins may be preferrable if the experiment is to be repeated.
