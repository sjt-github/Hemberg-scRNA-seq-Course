...
molecular identifiers (UMIs) and ERCC _spike-ins_ were used. For our purposes you need to download the files [`annotation.txt`](http://hemberg-lab.github.io/scRNA.seq.course/blischak/annotation.txt), [`molecules.txt`](http://hemberg-lab.github.io/scRNA.seq.course/blischak/molecules.txt), and [`reads.txt`](http://hemberg-lab.github.io/scRNA.seq.course/blischak/reads.txt) into the `blischak` folder in your working directory. These files are the copies of the original files made on the 15/03/16. We will use these copies for reproducibility purposes.
...
Another option in scater is to use your own filters. We define filters based on our previous analysis:
...
In addition to removing cells with poor quality, it is usually a good idea to exclude genes where we suspect that technical artefacts may have skewed the results. Moreover, inspection of the gene expression profiles may provide insights about how the experimental procedures could be imporved.
...
The distribution relatively flat indicating (but not guaraneeing!) good coverage of the full transcriptome of these cells. However, there are several spike-ins in the top 15 genes which suggests a greater dilution of the spike-ins may be preferrable if the experiment is to be repeated.
