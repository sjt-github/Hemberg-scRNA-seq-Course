...
plot(cellLabels[as.numeric(pseudotime_order_tscan)], xlab="Pseudotime Order", ylab="Timepoint");
```
...
# Store the ordering
pseudotime_monocle <- data.frame(Timepoint = phenoData(dCellDataSet)$timepoint, 
...
```{r monocle-vs-truth}
plot(pseudotime_monocle$pseudotime, pseudotime_monocle$Timepoint, xlab="Pseudotime", ylab="Timepoint")
```
...
timepoint_ordered = pseudotime_monocle[order(pseudotime_monocle$time), 1]
colours = rainbow(n=10) # red = early, violet = late

plot(matched_ordering, xlab = "Monocle Order", ylab = "TSCAN Order", col=colours[timepoint_ordered], pch=16)
```
...
TSCAN:
```{r Pou5f1-tscan}
TSCAN::singlegeneplot(deng[rownames(deng)=="Obox6",], dengorderTSCAN)
```
Monocle:
```{r Pou5f1-monocle}
plot_genes_in_pseudotime(dCellDataSet[fData(dCellDataSet)$gene == "Obox6",], color_by="timepoint")
```
